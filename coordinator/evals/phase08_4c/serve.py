...
from .doubles import DeliveryInvestigationDouble
...
def main():
    ...
    p.add_argument('--variant',choices=VARIANTS,default='base')
    p.add_argument('--source-port',type=int,default=18042);p.add_argument('--mcp-port',type=int,default=19042)
    p.add_argument('--host-port',type=int,default=18087);p.add_argument('--ui-origin',default='http://127.0.0.1:5178')
    p.add_argument('--delay',type=float,default=.35)
    args=p.parse_args();root=PROJECT/'.cache/phase08-4c/browser'/uuid4().hex
    root.mkdir(parents=True);settings=Settings(root/'source.sqlite3',root,True);prepare(settings,args.variant);install_standard(settings)
    with network(settings,args.source_port,args.mcp_port) as (source_url,mcp_url):
        ...
        delivery=DeliveryInvestigationDouble(config,delay=args.delay)
        quality=QualityInvestigationDouble(config,delay=args.delay)
        ...
        async def investigate(incoming,run,checkpoint):
            return await (delivery if incoming.event.change_id=='DR-009' else quality if incoming.event.change_id=='QE-004' else standard if incoming.event.change_id=='CR-019' else material)(incoming,run,checkpoint)
        ...
        if args.variant=='partial_failure':
            from program_coordinator.application.execution_models import SourceFailure
            original=source.delivery_link
            def fail_once(body,key):
                source.delivery_link=original
                raise SourceFailure('SOURCE_WRITE_REJECTED')
            source.delivery_link=fail_once
        if args.variant=='stale_proposal':
            from .fixtures import mutate
            original=source.delivery_prepare
            def stale(body,key):
                value=original(body,key)
                mutate(settings,lambda store:store.update('erp.delivery_state','STATE-DR-009',{'record_version':2}))
                return value
            source.delivery_prepare=stale
