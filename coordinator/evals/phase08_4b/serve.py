...
from .doubles import QualityInvestigationDouble
...
def main():
    ...
    p.add_argument('--variant',choices=VARIANTS,default='comparable')
    p.add_argument('--source-port',type=int,default=18041);p.add_argument('--mcp-port',type=int,default=19041)
    p.add_argument('--host-port',type=int,default=18086);p.add_argument('--ui-origin',default='http://127.0.0.1:5177')
    p.add_argument('--delay',type=float,default=.35)
    args=p.parse_args();root=PROJECT/'.cache/phase08-4b/browser'/uuid4().hex
    ...
    with network(settings,args.source_port,args.mcp_port) as (source_url,mcp_url):
        ...
        async def investigate(incoming,run,checkpoint):
            return await (quality if incoming.event.change_id=='QE-004' else standard if incoming.event.change_id=='CR-019' else material)(incoming,run,checkpoint)
